...
from apps.users.models import BaseModel, JpaUsers
...
class Messages(BaseModel):
    ...
    sendID = models.ForeignKey(verbose_name="发送者", to=JpaUsers, on_delete=models.CASCADE, related_name="sender")
    recID = models.ForeignKey(verbose_name="接收者", to=JpaUsers, on_delete=models.CASCADE, null=True, blank=True,
...
class MessageSysStatus(BaseModel):
    ...
    recID = models.ForeignKey(verbose_name="用户", to=JpaUsers, on_delete=models.CASCADE)
...
def getNewSysMessageNum(user: JpaUsers) -> int:
    admin: JpaUsers = None
    try:
    ...
    num_sys = 0
    msg_list = Messages.objects.filter(Q(sendID=None) | Q(sendID=admin)).filter(Q(recID=None) | Q(recID=user))
    for msg in msg_list:
...
def getPrivateMessage(user: JpaUsers, if_new: int = 0, people: str = "", start: int = 0, limit: int = -1,
                      id: int = -1) -> dict:
    people_flag = False
    admin: JpaUsers = None
    try:
    ...
    if people == "":
        condition = (~Q(sendID=None) & ~Q(sendID=admin) & Q(recID=user)) | (
                Q(sendID=user) & ~Q(recID=admin) & ~Q(recID=None))
    else:
        people_flag = True
        try:
            sender = JpaUsers.objects.get(username=people)
        except Exception as e:
            # status 101 获取发送者失败
...
def FilterMessage(user: JpaUsers, type: str, subtype: str, if_new: int = 0, id: int = -1) -> dict:
    ...
    for msg in msg_list:
        msg_dict = {"msg_id": msg.id, 'type': msg.type, 'subtype': msg.subtype, "title": msg.text.title,
                    "content": msg.text.content, "add_time": msg.add_time.timestamp(), "status": False,
                    "extra": msg.extra}
        if msg.sendID == admin or msg.sendID is None:
            # 系统通知
            new_flag = True
            msg_list2 = MessageSysStatus.objects.filter(message=msg).filter(recID=user)
            if len(msg_list2) != 0:
